...
{% mermaid %}
graph TD
    subgraph Model ["TCP/IP 五层模型"]
        A["应用层<br/><span style='font-size:11px;opacity:0.75'>HTTP, DNS, SSH, MQTT</span>"]
        B["传输层<br/><span style='font-size:11px;opacity:0.75'>TCP, UDP</span>"]
        C["网络层 (互联网层)<br/><span style='font-size:11px;opacity:0.75'>IP, ICMP, ARP</span>"]
        D["数据链路层<br/><span style='font-size:11px;opacity:0.75'>Ethernet, Wi-Fi (MAC)</span>"]
        E["物理层<br/><span style='font-size:11px;opacity:0.75'>光纤, 双绞线, 无线电波</span>"]
    ...
    %% 层次化色彩分级（自上而下冷暖/明暗平滑过渡，适配深浅背景）
    classDef l5 fill:#312e81,stroke:#6366f1,stroke-width:1.5px,color:#e0e7ff;
    classDef l4 fill:#1e3a8a,stroke:#3b82f6,stroke-width:1.5px,color:#dbeafe;
    classDef l3 fill:#0c4a6e,stroke:#0284c7,stroke-width:1.5px,color:#e0f2fe;
    classDef l2 fill:#134e4a,stroke:#0d9488,stroke-width:1.5px,color:#ccfbf1;
    classDef l1 fill:#1c1917,stroke:#78716c,stroke-width:1.5px,color:#f5f5f4;
    ...
    %% 外层容器样式
    style Model fill:#0f172a80,stroke:#64748b,stroke-dasharray: 4 4,color:#cbd5e1;
{% endmermaid %}
...
{% mermaid %}
graph TD
    subgraph S["发送方主机 (封装 Encapsulation)"]
        S_App["应用层：原始数据 (Data)"]
        S_Seg["传输层：报文段 / 数据报 (Segment)"]
        S_Pkt["网络层：数据包 (Packet)"]
        S_Frm["数据链路层：数据帧 (Frame)"]
        S_Bit["物理层：比特流 (Bits)"]

        S_App -->|添加 TCP/UDP 头部| S_Seg
        S_Seg -->|添加 IP 头部| S_Pkt
        S_Pkt -->|添加以太网头部/尾部| S_Frm
        S_Frm -->|转换为信号| S_Bit
    end

    Media["物理传输介质 (光纤 / 双绞线 / 无线电波)"]

    subgraph R["接收方主机 (解封装 Decapsulation)"]
        R_Bit["物理层：比特流 (Bits)"]
        R_Frm["数据链路层：数据帧 (Frame)"]
        R_Pkt["网络层：数据包 (Packet)"]
        R_Seg["传输层：报文段 / 数据报 (Segment)"]
        R_App["应用层：还原数据 (Data)"]

        R_Bit -->|信号解析成帧| R_Frm
        R_Frm -->|去除以太网头部/尾部| R_Pkt
        R_Pkt -->|去除 IP 头部| R_Seg
        R_Seg -->|去除 TCP/UDP 头部| R_App
    end
    ...
    %% 深色主题高亮样式
    style S fill:#0f172a,stroke:#3b82f6,stroke-width:2px,color:#93c5fd
    style R fill:#0f172a,stroke:#10b981,stroke-width:2px,color:#6ee7b7
    style Media fill:#312e81,stroke:#818cf8,stroke-width:2px,stroke-dasharray: 5 5,color:#e0e7ff
{% endmermaid %}
