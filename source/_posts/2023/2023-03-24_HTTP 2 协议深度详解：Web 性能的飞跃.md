...
{% mermaid %}
graph TD
    subgraph TCP["单一 TCP 物理连接 (交错传输二进制帧)"]
        direction LR
        FrameA["帧 A: Stream 1 (Header)"] --> FrameB["帧 B: Stream 2 (Header)"]
        FrameB --> FrameC["帧 C: Stream 1 (Data)"]
        FrameC --> FrameD["帧 D: Stream 2 (Data)"]
        FrameD --> FrameE["帧 E: Stream 3 (Header)"]
        FrameE --> FrameF["帧 F: Stream 1 (Data)"]
    end

    subgraph App["应用层逻辑视角 (独立双向流重组)"]
        direction TB
        subgraph Stream1["Stream 1"]
            Req1["请求 1 (Headers + Data)"]
            Resp1["响应 1 (Data)"]
        end
    ...
    %% 容器深色底色
    style TCP fill:#0f172a,stroke:#475569,stroke-width:1.5px,color:#94a3b8
    style App fill:#0f172a,stroke:#475569,stroke-width:1.5px,color:#94a3b8

    %% Stream 1 色系（天蓝）
    style Stream1 fill:#082f49,stroke:#0284c7,stroke-width:1px,color:#7dd3fc
    style FrameA fill:#0c4a6e,stroke:#38bdf8,stroke-width:1.5px,color:#f0f9ff
    style FrameC fill:#0c4a6e,stroke:#38bdf8,stroke-width:1.5px,color:#f0f9ff
    style FrameF fill:#0c4a6e,stroke:#38bdf8,stroke-width:1.5px,color:#f0f9ff
    style Req1 fill:#1e293b,stroke:#38bdf8,color:#e0f2fe
    style Resp1 fill:#1e293b,stroke:#38bdf8,color:#e0f2fe

    %% Stream 2 色系（翡翠绿）
    style Stream2 fill:#064e3b,stroke:#059669,stroke-width:1px,color:#6ee7b7
    style FrameB fill:#065f46,stroke:#34d399,stroke-width:1.5px,color:#ecfdf5
    style FrameD fill:#065f46,stroke:#34d399,stroke-width:1.5px,color:#ecfdf5
    style Req2 fill:#1e293b,stroke:#34d399,color:#d1fae5
    style Resp2 fill:#1e293b,stroke:#34d399,color:#d1fae5

    %% Stream 3 色系（琥珀橙）
    style Stream3 fill:#451a03,stroke:#d97706,stroke-width:1px,color:#fcd34d
    style FrameE fill:#78350f,stroke:#fbbf24,stroke-width:1.5px,color:#fffbeb
    style Req3 fill:#1e293b,stroke:#fbbf24,color:#fef3c7

    %% 连线颜色区分
    linkStyle 5,6,7 stroke:#38bdf8,stroke-width:2px;
    linkStyle 8,9 stroke:#34d399,stroke-width:2px;
    linkStyle 10 stroke:#fbbf24,stroke-width:2px;
{% endmermaid %}
