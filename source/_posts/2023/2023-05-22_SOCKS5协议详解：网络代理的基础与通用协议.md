...
sequenceDiagram
    autonumber
    actor App as 客户端应用
    participant Client as SOCKS5 客户端<br/>(本地代理)
    participant Server as SOCKS5 代理服务器
    participant Target as 目标服务器<br/>(Web/API)

    rect rgba(51, 65, 85, 0.25)
    Note over App, Server: 阶段一：握手与认证协商
    App->>+Client: 发起代理连接
    Client->>+Server: 协商认证方式 (VER=0x05, NMETHODS, METHODS)
    Server->>-Client: 选定认证方法 (VER=0x05, METHOD)
    
    alt 需要身份验证 (如 METHOD = 0x02)
        Client->>+Server: 提交凭据 (USERNAME, PASSWORD)
        Server->>Server: 验证身份
        Server->>-Client: 认证结果 (STATUS, 0x00=成功)
    end
    end

    rect rgba(30, 58, 138, 0.25)
    Note over Client, Target: 阶段二：建立目标连接 (TCP CONNECT)
    Client->>+Server: 请求建立连接 (CMD=0x01, ATYP, DST.ADDR, DST.PORT)
    Server->>+Target: 发起 TCP 三次握手
    Target-->>-Server: TCP 连接建立成功
    Server->>-Client: 响应请求结果 (REP=0x00 成功, BND.ADDR, BND.PORT)
    end

    rect rgba(15, 23, 42, 0.35)
    Note over App, Target: 阶段三：数据透明传输 / 转发
    App->>Client: 发送业务载荷 (明文/TLS)
    Client->>Server: 转发数据流
    Server->>Target: 转发至目标服务
    Target-->>Server: 服务端返回响应
    Server-->>Client: 转发响应数据
    Client-->>-App: 交付响应数据
    end
{% endmermaid %}
