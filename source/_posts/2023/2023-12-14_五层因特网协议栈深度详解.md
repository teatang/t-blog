...
graph TD
    subgraph Sender["发送方：封装过程 (Encapsulation)"]
        direction TB
        S_App["应用层 Application<br/>PDU: 数据 (Data)"]
        S_Tran["传输层 Transport<br/>PDU: 报文段 (Segment)"]
        S_Net["网络层 Network<br/>PDU: 数据包 (Packet)"]
        S_Link["数据链路层 Data Link<br/>PDU: 帧 (Frame)"]
        S_Phy["物理层 Physical<br/>PDU: 比特流 (Bits)"]

        S_App ==>|"添加 TCP/UDP 首部"| S_Tran
        S_Tran ==>|"添加 IP 首部"| S_Net
        S_Net ==>|"添加 MAC 首部与尾部"| S_Link
        S_Link ==>|"调制为光电/无线信号"| S_Phy
    end

    subgraph Receiver["接收方：解封装过程 (Decapsulation)"]
        direction TB
        R_App["应用层 Application<br/>PDU: 还原数据 (Data)"]
        R_Tran["传输层 Transport<br/>PDU: 端口分流与保序 (Segment)"]
        R_Net["网络层 Network<br/>PDU: 路由验证 (Packet)"]
        R_Link["数据链路层 Data Link<br/>PDU: 校验 MAC 并剥离首尾 (Frame)"]
        R_Phy["物理层 Physical<br/>PDU: 还原比特流 (Bits)"]

        R_Phy ==>|"解析比特流"| R_Link
        R_Link ==>|"剥离 MAC 首部/尾部"| R_Net
        R_Net ==>|"剥离 IP 首部"| R_Tran
        R_Tran ==>|"解析端口并交付"| R_App
    end

    %% 物理信道底座
    S_Phy ==>|"物理介质 / 光纤 / 无线电波"| R_Phy

    %% 逻辑对等层交互（横向对齐映射）
    S_App -.->|"虚拟对等通信 (App-to-App)"| R_App
    S_Tran -.->|"端到端可靠连接 (Port-to-Port)"| R_Tran
    S_Net -.->|"逻辑逐跳路由寻址 (IP-to-IP)"| R_Net
    ...
    %% 发送端节点样式
    style S_App fill:#1e293b,stroke:#60a5fa,color:#f8fafc
    style S_Tran fill:#0c4a6e,stroke:#38bdf8,stroke-width:1.5px,color:#f0f9ff
    style S_Net fill:#1e1b4b,stroke:#818cf8,stroke-width:1.5px,color:#e0e7ff
    style S_Link fill:#064e3b,stroke:#34d399,stroke-width:1.5px,color:#ecfdf5
    style S_Phy fill:#334155,stroke:#94a3b8,color:#f1f5f9

    %% 接收端节点样式
    style R_App fill:#1e293b,stroke:#34d399,color:#f8fafc
    style R_Tran fill:#0c4a6e,stroke:#38bdf8,stroke-width:1.5px,color:#f0f9ff
    style R_Net fill:#1e1b4b,stroke:#818cf8,stroke-width:1.5px,color:#e0e7ff
    style R_Link fill:#064e3b,stroke:#34d399,stroke-width:1.5px,color:#ecfdf5
    style R_Phy fill:#334155,stroke:#94a3b8,color:#f1f5f9

    %% 对等层虚线色调
    linkStyle 9 stroke:#60a5fa,stroke-dasharray: 4 4;
    linkStyle 10 stroke:#38bdf8,stroke-dasharray: 4 4;
    linkStyle 11 stroke:#818cf8,stroke-dasharray: 4 4;
{% endmermaid %}
