...
sequenceDiagram
    ...
    box rgba(15, 23, 42, 0.6) 本地客户端 (Client)
        participant Client as 💻 Xray 客户端 (REALITY)
    end

    box rgba(23, 17, 37, 0.6) 节点与伪装目标 (Server & Decoy)
        participant Server as 🚀 REALITY 服务端 (Xray-core)
        participant Decoy as 🏢 目标伪装站 (e.g., Apple / Microsoft)
        participant Target as 🌐 目标网站 (Target Server)
    end

    %% 阶段 1: 特殊 ClientHello 与透明分流
    Note over Client,Server: 阶段一：首包发起源鉴权 (无需自备证书，借壳真实指纹)
    Client->>Server: TCP 三次握手 (目标端口 443)
    Server-->>Client: TCP 连接就绪

    Client->>Server: 发送 ClientHello<br/>[伪装 SNI + uTLS 浏览器指纹 + 认证密文(含 ShortId)]
    Server->>Server: 用服务端私钥尝试 ECDH 解密并比对 ShortId

    alt 校验失败 (GFW 主动嗅探 / 普通浏览器访问)
        Note over Server,Decoy: 阶段二 A：透明回落转发 (偷梁换柱，返回绝对合法的真实证书)
        Server->>Decoy: 100% 透明转发原始 ClientHello
        Decoy-->>Server: 返回真证书、ServerHello 等握手数据
        Server-->>Client: 原封不动回传探测者 (无任何协议破绽)
    else 校验成功 (确认是受信任的合法客户端)
        Note over Client,Server: 阶段二 B：截获握手，本地派生加密隧道
        Server->>Server: 截断对 Decoy 的转发，本地完成 TLS 握手封装
        Server-->>Client: 返回 REALITY 专属 ServerHello 与加密确认
        Client->>Client: 验证服务端真实性，协商会话主密钥 (Session Key)

        %% 阶段 3: 建立 VLESS 代理会话与数据中继
        Note over Client,Target: 阶段三：VLESS + XTLS-Vision 双向加密传输
        rect rgba(15, 23, 42, 0.5)
            Client->>Server: 发送 VLESS UUID 认证 + 原始应用载荷 (Application Data)
            Server->>Target: 转发原始请求至目标网络服务
            Target-->>Server: 返回业务响应数据
            Server-->>Client: 回传加密响应载荷 (支持 Vision 零拷贝直通)
        end
    end
{% endmermaid %}
