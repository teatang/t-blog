...
sequenceDiagram
    ...
    box rgba(15, 23, 42, 0.6) 客户端本地环境 (Local)
        participant Client as 💻 本地客户端 (v2ray-core)
    end

    box rgba(23, 17, 37, 0.6) 境外服务器与目标 (Remote)
        participant Server as 🚀 远端节点 (Nginx / V2Ray)
        participant Target as 🌐 目标网站 (Web Server)
    end

    %% 阶段 1: 传输层与 TLS 伪装隧道
    Note over Client,Server: 阶段一：建立底层标准 Web 通信（审查者视作正常 HTTPS 访问）
    Client->>+Server: 1. TCP 三次握手 (目标端口 443)
    Server-->>-Client: TCP 连接建立成功 (SYN-ACK)

    Client->>+Server: 2. 标准 TLS 握手 (Client Hello / SNI 伪装域名)
    Server-->>-Client: TLS 证书确认并完成加密握手 (ALPN: http/1.1)

    %% 阶段 2: 协议升格为 WebSocket
    Note over Client,Server: 阶段二：在 TLS 隧道内发起 HTTP/WebSocket 协议升级
    Client->>+Server: 3. HTTP GET Upgrade: websocket (携带指定 Path)
    Server-->>-Client: HTTP 101 Switching Protocols (WS 隧道就绪)

    %% 阶段 3: VMess 认证握手与中继
    Note over Client,Server: 阶段三：在 WS/TLS 隧道中传输 VMess AEAD 认证与指令
    Client->>+Server: 4. 发送 VMess 认证头 (UUID 校验码 + 时间戳 + 目标地址加密包)
    Server->>Server: 5. 校验时间戳 (防重放) 并使用 UUID 解密请求头

    alt 认证失败 / 非法探测 / 非法路径
        Server-->>Client: 回退伪装网站内容或直接 RST 断开连接
    else 认证成功并提取目标地址
        Server->>+Target: 6. 建立到目标站点的真实 TCP/UDP 连接
        Target-->>-Server: 目标服务连接就绪
        
        %% 阶段 4: 双向加密数据流
        Note over Client,Target: 阶段四：双向载荷传输 (数据逐层封装解封)
        rect rgba(15, 23, 42, 0.5)
            loop 业务数据双向流动 (TCP Payload)
                Client->>+Server: [VMess Payload] 封装进 WS 二进制帧 ➔ 经 TLS 加密发出
                Server->>Server: 剥离 TLS ➔ 剥离 WS 报头 ➔ VMess 解密还原
                Server->>+Target: 发送原始应用层请求 (HTTP/DNS/Stream)

                Target-->>-Server: 返回业务响应数据
                Server->>Server: VMess 加密 ➔ 打包 WS 帧 ➔ TLS 加密封装
                Server-->>-Client: [VMess Payload] 经加密隧道回传客户端
                Client->>Client: 逐层解密还原并交付本地应用
            end
        end
    end
{% endmermaid %}
