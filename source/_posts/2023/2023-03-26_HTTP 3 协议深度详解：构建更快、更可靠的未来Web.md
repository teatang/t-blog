...
{% mermaid %}
graph TD
    subgraph Arch["端到端架构交互流程 (End-to-End Flow)"]
        User["终端用户"] --> Browser["Web 浏览器"]
        Browser -->|"HTTP/3 请求与响应"| ClientQUIC["QUIC 客户端模块"]
        ClientQUIC ==>|"QUIC 帧 (内置 TLS 1.3 加密) over UDP"| ServerQUIC["QUIC 服务端模块"]
        ServerQUIC -->|"HTTP/3 解码请求"| OriginServer["源站 Web 服务器"]
    end

    subgraph Stack["HTTP/3 核心协议栈架构 (Protocol Stack)"]
        direction LR
        L_App["应用层：HTTP/3"] -->|"QPACK 头部压缩"| L_QUIC["传输层：QUIC<br/>(原生多路复用 / 0-RTT / 连接迁移)"]
        L_QUIC -->|"封装为 UDP 数据报"| L_UDP["传输层基础：UDP (端口寻址)"]
        L_UDP -->|"IP 路由与分片"| L_IP["网络层：IP (IPv4 / IPv6)"]
    end

    %% 业务流与协议栈关联
    ClientQUIC -.->|"协议规范支撑"| Stack

    %% 容器深色面板
    style Arch fill:#0f172a,stroke:#334155,stroke-width:1.5px,color:#94a3b8
    style Stack fill:#0f172a,stroke:#38bdf8,stroke-width:1.5px,color:#93c5fd

    %% 节点精细配色
    style User fill:#1e293b,stroke:#64748b,color:#f8fafc
    style Browser fill:#1e293b,stroke:#64748b,color:#f8fafc
    style OriginServer fill:#1e293b,stroke:#64748b,color:#f8fafc

    %% QUIC 核心通道高亮（青蓝与翡翠绿）
    style ClientQUIC fill:#0c4a6e,stroke:#38bdf8,stroke-width:2px,color:#f0f9ff
    style ServerQUIC fill:#064e3b,stroke:#34d399,stroke-width:2px,color:#ecfdf5

    %% 协议栈层级渐变色
    style L_App fill:#1e1b4b,stroke:#818cf8,stroke-width:1.5px,color:#e0e7ff
    style L_QUIC fill:#0c4a6e,stroke:#38bdf8,stroke-width:2px,color:#f0f9ff
    style L_UDP fill:#1e293b,stroke:#94a3b8,stroke-width:1.5px,color:#f1f5f9
    style L_IP fill:#0f172a,stroke:#64748b,stroke-width:1.5px,color:#cbd5e1
{% endmermaid %}
