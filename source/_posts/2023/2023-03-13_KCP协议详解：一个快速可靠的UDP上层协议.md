...
{% mermaid %}
graph TD
    subgraph TCP["传统 TCP：严格保序导致长时间队头阻塞"]
        direction TB
        T_P1["Packet 1"] -->|"正常到达"| T_B1["接收缓冲 1"] --> T_App1["递交 App"]
        T_P2["Packet 2"] -->|"正常到达"| T_B2["接收缓冲 2"] --> T_App1
        T_P3["Packet 3"] -.->|"丢包 Drop"| T_Lost["❌ 丢包 Gap"]
        T_P4["Packet 4"] -->|"提前到达"| T_B4["缓存等待 Waiting"]
        T_P5["Packet 5"] -->|"提前到达"| T_B5["缓存等待 Waiting"]

        T_Lost -.->|"超时等待 RTO 翻倍 / 累积确认延误"| T_Retrans["超时重传 Packet 3"]
        T_Retrans ==>|"迟到的修复"| T_B3["接收缓冲 3"]

        T_B3 --> T_Unblock["解除阻塞：按序递交"]
        T_B4 --> T_Unblock
        T_B5 --> T_Unblock
        T_Unblock --> T_App2["递交 App - 高抖动/高延迟"]
    end

    subgraph KCP["KCP 优化：选择性确认 + 快速重传大幅压缩阻塞窗口"]
        direction TB
        K_P1["Packet 1"] -->|"正常到达"| K_B1["接收缓冲 1"] --> K_App1["递交 App"]
        K_P2["Packet 2"] -->|"正常到达"| K_B2["接收缓冲 2"] --> K_App1
        K_P3["Packet 3"] -.->|"丢包 Drop"| K_Lost["❌ 丢包 Gap"]
        K_P4["Packet 4"] -->|"UNA 与 ACK"| K_B4["乱序缓存 UNA 3, ACK 4"]
        K_P5["Packet 5"] -->|"UNA 与 ACK"| K_B5["乱序缓存 UNA 3, ACK 5"]

        K_Lost -.->|"跳过慢启动 / 触发 Fast Retransmit"| K_FastR["立即快速重传 Packet 3"]
        K_FastR ==>|"极速补全缺口 - 极低延迟"| K_B3["接收缓冲 3"]

        K_B3 --> K_Deliver["极速重排并递交"]
        K_B4 --> K_Deliver
        K_B5 --> K_Deliver
        K_Deliver --> K_App2["递交 App - 平滑低抖动"]
    end

    %% 深色主题样式注入
    style TCP fill:#0f172a,stroke:#ef4444,stroke-width:1.5px,color:#fca5a5
    style KCP fill:#0f172a,stroke:#10b981,stroke-width:1.5px,color:#6ee7b7
    
    style T_Lost fill:#450a0a,stroke:#f87171,stroke-width:1px,color:#fecaca
    style K_Lost fill:#450a0a,stroke:#f87171,stroke-width:1px,color:#fecaca
    
    style T_Retrans fill:#3b1d11,stroke:#fb923c,stroke-width:1px,color:#ffedd5
    style K_FastR fill:#064e3b,stroke:#34d399,stroke-width:1px,color:#d1fae5
    
    style T_Unblock fill:#1e293b,stroke:#94a3b8,color:#cbd5e1
    style K_Deliver fill:#1e293b,stroke:#38bdf8,color:#e0f2fe
{% endmermaid %}
