...
sequenceDiagram
    ...
    rect rgb(30, 40, 60)
        Note over C,S: TCP 三次握手 (3-Way Handshake)
    ...
    Note over C,S: 全双工 TCP 字节流通道建立就绪，可进行数据传输
{% endmermaid %}
...
sequenceDiagram
    ...
    rect rgb(50, 30, 40)
        Note over C,S: 阶段 1: 客户端主动发起半关闭 (Client Half-Close)
        C->>S: [FIN, ACK] seq = u, ack = v
        Note left of C: State: FIN_WAIT_1
    ...
    rect rgb(40, 45, 60)
        Note over C,S: 阶段 2: 服务端单向数据传输 (可选)
        S-->>C: [Data] 传输剩余未完成的数据帧...
    end

    rect rgb(50, 30, 40)
        Note over C,S: 阶段 3: 服务端发起全关闭 (Server Final-Close)
        S->>C: [FIN, ACK] seq = w, ack = u + 1
        Note right of S: State: LAST_ACK
        
        C->>S: [ACK] seq = u + 1, ack = w + 1
        Note left of C: State: TIME_WAIT (2MSL Timer)
        Note right of S: State: CLOSED
    end

    Note left of C: 2MSL 超时后 -> State: CLOSED
{% endmermaid %}
