...
graph LR
    %% 第一层级：IP 报文分解
    subgraph IP_Layer["IP 数据报结构 (IPv4 Datagram)"]
        direction TB
        IP_Pkt["IP 数据报"]
        IP_Hdr["IP 首部 (Header: 20-60 Bytes)<br/>协议字段 Protocol = 1 (ICMP)"]
        IP_Payload["IP 数据载荷 (Data Payload)"]

        IP_Pkt --> IP_Hdr
        IP_Pkt ==>|"封装载荷"| IP_Payload
    end

    %% 第二层级：ICMP 结构
    subgraph ICMP_Layer["ICMP 报文完整结构 (ICMP Message)"]
        direction TB
        ICMP_Pkt["ICMP 报文整体"]
        
        ICMP_Type["Type 类型 (1 Byte)<br/>区分差错报告 / 查询请求"]
        ICMP_Code["Code 代码 (1 Byte)<br/>进一步细分错误原因"]
        ICMP_Sum["Checksum 校验和 (2 Bytes)<br/>覆盖整个 ICMP 报文"]
        ICMP_Rest["Rest of Header (4 Bytes)<br/>特定类型首部内容"]
        ICMP_Data["Data 数据载荷 (可变长度)"]
    ...
    %% 第三层级：可变首部与差错载荷详细展开
    subgraph Sub_Rest["Rest of Header 细节"]
        direction TB
        R_Echo["查询报文 (Echo Request/Reply)<br/>Identifier (2B) + Sequence (2B)"]
        R_Unreach["差错报文 (如 Destination Unreachable)<br/>Unused (4 Bytes 保留为 0)"]
        R_Redirect["重定向报文 (Redirect)<br/>Gateway IP Address (4 Bytes)"]
    end

    subgraph Sub_Data["差错报文的 Data 载荷规范 (RFC 792)"]
        direction TB
        D_IpHdr["导致错误的原始 IP 首部 (Original IP Header)"]
        D_IpData["原始 IP 数据前 8 字节<br/>(含传输层 TCP/UDP 源与目的端口)"]
    end

    %% 跨层级级联关系
    IP_Payload ==>|"承载"| ICMP_Pkt
    ICMP_Rest -->|"依据 Type 细分"| R_Echo
    ICMP_Rest -->|"依据 Type 细分"| R_Unreach
    ICMP_Rest -->|"依据 Type 细分"| R_Redirect
    ICMP_Data -->|"仅差错报告报文附带"| D_IpHdr
    ICMP_Data -->|"仅差错报告报文附带"| D_IpData

    %% 深色面板样式
    style IP_Layer fill:#0f172a,stroke:#334155,stroke-width:1.5px,color:#94a3b8
    style ICMP_Layer fill:#0f172a,stroke:#38bdf8,stroke-width:1.5px,color:#93c5fd
    style Sub_Rest fill:#111827,stroke:#64748b,stroke-width:1px,color:#cbd5e1
    style Sub_Data fill:#111827,stroke:#64748b,stroke-width:1px,color:#cbd5e1

    %% 关键节点深色高亮
    style IP_Pkt fill:#1e293b,stroke:#64748b,color:#f8fafc
    style IP_Hdr fill:#1e293b,stroke:#64748b,color:#cbd5e1
    style IP_Payload fill:#0c4a6e,stroke:#38bdf8,stroke-width:2px,color:#f0f9ff

    style ICMP_Pkt fill:#0c4a6e,stroke:#38bdf8,stroke-width:2px,color:#f0f9ff
    style ICMP_Type fill:#1e1b4b,stroke:#818cf8,color:#e0e7ff
    style ICMP_Code fill:#1e1b4b,stroke:#818cf8,color:#e0e7ff
    style ICMP_Sum fill:#1e293b,stroke:#94a3b8,color:#f8fafc
    style ICMP_Rest fill:#064e3b,stroke:#34d399,stroke-width:1.5px,color:#ecfdf5
    style ICMP_Data fill:#451a03,stroke:#fb923c,stroke-width:1.5px,color:#ffedd5

    style R_Echo fill:#064e3b,stroke:#34d399,color:#d1fae5
    style R_Unreach fill:#1e293b,stroke:#64748b,color:#cbd5e1
    style R_Redirect fill:#1e293b,stroke:#64748b,color:#cbd5e1

    style D_IpHdr fill:#451a03,stroke:#fb923c,color:#ffedd5
    style D_IpData fill:#451a03,stroke:#fb923c,color:#ffedd5
{% endmermaid %}
