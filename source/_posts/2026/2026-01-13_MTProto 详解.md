...
{% mermaid %}
graph TD
    subgraph API_Layer["1. API 与序列化层 (TL-Schema Layer)"]
        direction TB
        API_1["API 方法调用<br/>(Client Requests)"]
        API_2["TL-Schema 序列化<br/>(二进制类型系统编解码)"]
        API_3["构造明文消息报文<br/>(Message: msg_id + seq_no)"]

        API_1 --> API_2 --> API_3
    end

    subgraph Crypto_Layer["2. MTProto 2.0 核心加密层 (Security Layer)"]
        direction TB
        subgraph KeyGen["前置密钥协商 (Auth Key)"]
            DH_1["Diffie-Hellman 密钥交换"] --> DH_2["生成 2048-bit 授权密钥<br/>(auth_key)"]
        end

        subgraph Encrypt["单消息加密流水线"]
            C_1["计算 msg_key<br/>(SHA-256 派生会话密钥)"]
            C_2["AES-256-IGE 模式对称加密<br/>(结合 msg_key, auth_key, IV)"]
            C_3["生成已加密载荷<br/>(auth_key_id + msg_key + 密文)"]
    ...
    subgraph Trans_Layer["3. 传输层 (Transport Protocol)"]
        direction TB
        T_1["添加传输层协议首部<br/>(Abridged / Intermediate / Padded)"]
        T_2["可选: Obfuscated2 混淆外壳<br/>(规避 DPI 流量指纹审查)"]
        T_3["底层物理传输<br/>(TCP / HTTP / WebSocket / UDP)"]

        T_1 --> T_2 --> T_3
    end

    %% 跨层级主数据流
    API_3 ==>|"递交明文 Payload"| C_1
    C_3 ==>|"递交已加密 MTProto 包"| T_1

    %% 容器深色面板
    style API_Layer fill:#0f172a,stroke:#3b82f6,stroke-width:1.5px,color:#93c5fd
    style Crypto_Layer fill:#1e1b4b,stroke:#818cf8,stroke-width:1.5px,color:#c7d2fe
    style Trans_Layer fill:#0f172a,stroke:#10b981,stroke-width:1.5px,color:#6ee7b7

    style KeyGen fill:#111827,stroke:#64748b,stroke-width:1px,color:#94a3b8
    style Encrypt fill:#111827,stroke:#818cf8,stroke-width:1px,color:#c7d2fe

    %% 节点样式
    style API_1 fill:#1e293b,stroke:#60a5fa,color:#f8fafc
    style API_2 fill:#1e293b,stroke:#60a5fa,color:#f8fafc
    style API_3 fill:#0c4a6e,stroke:#38bdf8,stroke-width:1.5px,color:#f0f9ff

    style DH_1 fill:#1e293b,stroke:#94a3b8,color:#cbd5e1
    style DH_2 fill:#312e81,stroke:#a5b4fc,color:#e0e7ff
    style C_1 fill:#312e81,stroke:#a5b4fc,color:#e0e7ff
    style C_2 fill:#0c4a6e,stroke:#38bdf8,stroke-width:2px,color:#f0f9ff
    style C_3 fill:#064e3b,stroke:#34d399,stroke-width:1.5px,color:#ecfdf5

    style T_1 fill:#1e293b,stroke:#64748b,color:#f8fafc
    style T_2 fill:#451a03,stroke:#fb923c,stroke-width:1.5px,color:#ffedd5
    style T_3 fill:#064e3b,stroke:#34d399,stroke-width:1.5px,color:#ecfdf5
{% endmermaid %}
