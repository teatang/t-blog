...
{% mermaid %}
%%{init: {'theme': 'dark', 'themeVariables': { 'primaryColor': '#2d333b', 'edgeLabelBackground':'#1c2128', 'tertiaryColor': '#1c2128'}}}%%
graph TD
    %% 节点定义
    UI["💻 用户界面 (Frontend)<br/><small>React / Vue / HTML</small>"]
    IPC["🔌 Tauri IPC Bridge<br/><small>安全序列化传递</small>"]
    Rust["🦀 Rust Core (Backend)<br/><small>逻辑与安全边界</small>"]
    OS_UI["🖥️ 原生组件<br/><small>窗口 / 菜单 / 通知</small>"]
    Sys["⚙️ 系统能力<br/><small>文件 / 网络 / 数据库</small>"]
    WV["🌐 WebView 容器<br/><small>WebView2 / WKWebView</small>"]
    ...
    %% 分组样式
    subgraph App_Scope ["Tauri Runtime 运行环境"]
        direction TB
        UI
        IPC
        Rust
    ...
    %% 样式美化
    style App_Scope fill:#161b22,stroke:#30363d,stroke-width:2px,color:#8b949e
    style UI fill:#1f6feb,stroke:#58a6ff,color:#fff
    style Rust fill:#d29922,stroke:#f0883e,color:#fff
    style IPC fill:#238636,stroke:#3fb950,color:#fff
    style WV fill:#21262d,stroke:#8b949e,stroke-dasharray: 5 5
{% endmermaid %}
