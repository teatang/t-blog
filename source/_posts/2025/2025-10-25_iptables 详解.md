...
{% mermaid %}
graph TD
    A(数据包进入网卡) --> B{"PREROUTING Chain (Mangle, Raw, Nat)"}
    B --> C{路由判断: 目标是本机还是转发?}
    C -- 目标是本机 --> D{"INPUT Chain (Mangle, Filter, Security)"}
    D --> E(本地进程接收)
    C -- 目标是转发 --> F{"FORWARD Chain (Mangle, Filter, Security)"}
    F --> G{"POSTROUTING Chain (Mangle, Nat)"}
    G --> H(数据包离开网卡)
    I(本地进程发送数据包) --> J{"OUTPUT Chain (Mangle, Raw, Nat, Filter, Security)"}
    J --> G
{% endmermaid %}
