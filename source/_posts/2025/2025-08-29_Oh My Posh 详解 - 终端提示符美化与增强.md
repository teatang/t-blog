...
flowchart TD
    %% 阶段 1：启动时一次性初始化
    subgraph Boot [" ⚡ 阶段 1: Shell 启动初始化 (只执行一次) "]
        Profile["📄 加载配置文件<br/><code>$PROFILE / .bashrc / .zshrc</code>"]
        InitCmd["⚙️ 执行初始化钩子注入<br/><code>oh-my-posh init [shell] --config [theme] | iex</code>"]
        Inject["💉 动态注册原生钩子<br/><code>function prompt { ... }</code>"]

        Profile --> InitCmd --> Inject
    end

    %% 阶段 2：交互循环（每次回车/新提示符）
    subgraph Trigger [" ⌨️ 阶段 2: 换行触发渲染 (每轮交互) "]
        NewLine["↩️ 触发新提示符行<br/><i>(用户回车 / 命令执行完毕)</i>"]
        CallPrompt["📞 Shell 调用注入的 <code>prompt</code> 函数"]
        SpawnOMP["🚀 后台拉起二进制程序<br/><code>oh-my-posh print primary</code>"]

        NewLine --> CallPrompt --> SpawnOMP
    end

    Inject -.->|"定义调用逻辑"| CallPrompt

    %% 阶段 3：Go 核心引擎管线
    subgraph Engine [" 🎨 阶段 3: Oh-My-Posh Go 核心渲染管线 "]
        direction TB
        LoadConfig["📖 <b>1. 解析主题配置</b><br/>读取 blocks / segments 布局与样式"]
        
        subgraph SegPipeline [" 2. 遍历并装配每个 Segment "]
            direction TB
            FetchCtx["🌐 <b>收集上下文数据</b><br/>Git 状态 · 虚拟环境 · 电量 · 退出码"]
            FormatSeg["🎨 <b>应用 ANSI 样式</b><br/>解析图标字模 (Nerd Font) + 渐变/背景色"]
            FetchCtx --> FormatSeg
        end

        Assemble["🧩 <b>3. 拼接装配</b><br/>合并所有 Block 与 Segment 为最终 ANSI 序列"]
    ...
    %% 阶段 4：终端呈现
    subgraph Render [" 🖥️ 阶段 4: 终端回显 "]
        Output["📤 标准输出 (stdout) 原始控制字符串"]
        Terminal["✨ 终端渲染美化后的提示符<br/><code>[ 📁 ~/repo |  main* | 🐍 v3.12 ] ❯ </code>"]
    ...
    %% 深色主题样式定制
    style Boot fill:#0f172a,stroke:#475569,stroke-dasharray: 4 4,color:#94a3b8
    style Trigger fill:#0c192c,stroke:#38bdf8,stroke-dasharray: 4 4,color:#93c5fd
    style Engine fill:#130b24,stroke:#c084fc,stroke-dasharray: 4 4,color:#e9d5ff
    style SegPipeline fill:#1a102e,stroke:#a855f7,stroke-width:1px,color:#d8b4fe
    style Render fill:#0d1d18,stroke:#22c55e,stroke-dasharray: 4 4,color:#86efac

    style Profile fill:#1e293b,stroke:#64748b,color:#cbd5e1
    style InitCmd fill:#1e293b,stroke:#0ea5e9,color:#f8fafc
    style Inject fill:#0369a1,stroke:#38bdf8,stroke-width:1.5px,color:#ffffff

    style NewLine fill:#1e293b,stroke:#64748b,color:#cbd5e1
    style CallPrompt fill:#1e293b,stroke:#38bdf8,stroke-width:1.2px,color:#f8fafc
    style SpawnOMP fill:#075985,stroke:#38bdf8,stroke-width:1.5px,color:#ffffff

    style LoadConfig fill:#1e293b,stroke:#a855f7,color:#f8fafc
    style FetchCtx fill:#1e293b,stroke:#f59e0b,stroke-width:1.2px,color:#fde68a
    style FormatSeg fill:#581c87,stroke:#c084fc,stroke-width:1.5px,color:#ffffff
    style Assemble fill:#4c1d95,stroke:#c084fc,stroke-width:1.5px,color:#ffffff

    style Output fill:#1e293b,stroke:#22c55e,color:#cbd5e1
    style Terminal fill:#14532d,stroke:#22c55e,stroke-width:2px,color:#f0fdf4
{% endmermaid %}
