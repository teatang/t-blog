...
graph TD
    %% 样式定义（深色 UI 适配）
    classDef byteCell fill:#1e293b,stroke:#475569,stroke-width:1px,color:#94a3b8;
    classDef dataChunk fill:#0f2937,stroke:#38bdf8,stroke-width:2px,color:#e0f2fe;
    classDef unalignedWarn fill:#450a0a,stroke:#ef4444,stroke-width:1.5px,color:#fecaca;
    classDef alignedGood fill:#064e3b,stroke:#10b981,stroke-width:1.5px,color:#d1fae5;
    classDef cpuNode fill:#1e1b4b,stroke:#818cf8,stroke-width:1.5px,color:#e0e7ff;

    %% ---------------- 未对齐访问 ----------------
    subgraph Sub_Unaligned ["⚠️ 未对齐访问 (Unaligned: 4 字节跨越字边界)"]
        direction TB

        U_Data["📦 数据 A (4 字节, 地址 1~4)"]:::dataChunk

        subgraph U_Mem ["物理内存 (按 4 字节字边界划分)"]
            direction LR
            subgraph U_W0 ["字 0 (地址 0-3)"]
                direction LR
                U_M0["0"]:::byteCell --- U_M1["<b>1</b>"]:::dataChunk --- U_M2["<b>2</b>"]:::dataChunk --- U_M3["<b>3</b>"]:::dataChunk
            end
            subgraph U_W1 ["字 1 (地址 4-7)"]
                direction LR
                U_M4["<b>4</b>"]:::dataChunk --- U_M5["5"]:::byteCell --- U_M6["6"]:::byteCell --- U_M7["7"]:::byteCell
            end
        end

        U_CPU["🖥️ CPU 内存控制器"]:::cpuNode
        U_R1["第 1 次总线读取: 字 0"]:::unalignedWarn
        U_R2["第 2 次总线读取: 字 1"]:::unalignedWarn
        U_Combine["移位 & 掩码拼接处理"]:::unalignedWarn
        U_Res["✅ 最终得到完整 4 字节数据"]:::byteCell

        U_Data -. 跨越字边界 .-> U_Mem
        U_CPU -->|读 0x00-0x03| U_R1
        U_CPU -->|读 0x04-0x07| U_R2
        U_R1 --> U_Combine
        U_R2 --> U_Combine
        U_Combine --> U_Res
    end
    ...
    %% ---------------- 对齐访问 ----------------
    subgraph Sub_Aligned ["✨ 对齐访问 (Aligned: 4 字节自然对齐到地址 4)"]
        direction TB

        A_Data["📦 数据 B (4 字节, 地址 4~7)"]:::dataChunk

        subgraph A_Mem ["物理内存 (按 4 字节字边界划分)"]
            direction LR
            subgraph A_W0 ["字 0 (地址 0-3)"]
                direction LR
                A_M0["0"]:::byteCell --- A_M1["1"]:::byteCell --- A_M2["2"]:::byteCell --- A_M3["3"]:::byteCell
            end
            subgraph A_W1 ["字 1 (地址 4-7)"]
                direction LR
                A_M4["<b>4</b>"]:::dataChunk --- A_M5["<b>5</b>"]:::dataChunk --- A_M6["<b>6</b>"]:::dataChunk --- A_M7["<b>7</b>"]:::dataChunk
            end
        end

        A_CPU["🖥️ CPU 内存控制器"]:::cpuNode
        A_R1["单次总线读取: 字 1 (1x Cycle)"]:::alignedGood
        A_Res["✅ 直接获取完整 4 字节数据"]:::alignedGood

        A_Data -. 恰好放入字 1 .-> A_Mem
        A_CPU -->|读 0x04-0x07| A_R1
        A_R1 -->|直接送入寄存器| A_Res
    end
    {% endmermaid %}
