...
sequenceDiagram
    autonumber
    participant DevA as 设备 A<br/>(MAC_A)
    box rgba(49, 50, 68, 0.45) 交换机系统
    participant Port1 as 端口 1
    participant Switch as 转发核心 (MAC表)
    participant Port2 as 端口 2
    participant OtherPorts as 其余各端口
    end
    participant DevB as 设备 B<br/>(MAC_B)

    DevA->>Port1: 数据帧 (Src: MAC_A, Dst: MAC_B)
    activate Port1
    Port1->>Switch: 投递帧至交换矩阵
    deactivate Port1
    activate Switch

    rect rgb(30, 30, 46)
    Note over Switch: MAC 地址学习流程
    Switch->>Switch: 1. 解析源 MAC (MAC_A)
    Switch->>Switch: 2. 绑定映射: MAC_A ↔ 端口 1
    Switch->>Switch: 3. 更新 / 刷新 MAC 转发表
    end

    Switch->>Switch: 4. 检索目的 MAC (MAC_B)

    alt MAC_B 存在于转发表 (单播精确转发)
        Switch->>Port2: 定向转发 (端口 2)
        deactivate Switch
        activate Port2
        Port2->>DevB: 送达目标设备
        deactivate Port2
    else MAC_B 未知或为广播地址 (泛洪 / Flooding)
        activate Switch
        par 转发至目标端口
            Switch->>Port2: 广播转发
            activate Port2
            Port2->>DevB: 接收帧 (如 ARP 请求)
            deactivate Port2
        and 转发至其余非源端口
            Switch->>OtherPorts: 泛洪转发 (除端口 1 外)
            activate OtherPorts
            OtherPorts-->>Switch: (丢弃或无响应)
            deactivate OtherPorts
        end
        deactivate Switch
    end
{% endmermaid %}
...
flowchart TD
    ...
    subgraph Core["核心层 (Core Layer)"]
        A["核心交换机<br/>Core-SW01"]
    end

    %% 汇聚层
    subgraph Dist["汇聚层 (Distribution Layer)"]
        B1["汇聚交换机 01<br/>Agg-SW01"]
        B2["汇聚交换机 02<br/>Agg-SW02"]
    end

    %% 接入层
    subgraph Access["接入层 (Access Layer)"]
        C1["接入交换机 01<br/>Acc-SW01"]
        C2["接入交换机 02<br/>Acc-SW02"]
        C3["接入交换机 03<br/>Acc-SW03"]
        C4["接入交换机 04<br/>Acc-SW04"]
    end

    %% 终端设备
    subgraph EndDev["终端设备 (End Devices)"]
        D1["PC-A"]
        D2["PC-B"]
        D3["Server-C"]
        D4["PC-D"]
    ...
    %% 深色主题专用样式定义 (Fill / Stroke / Font)
    classDef coreNode fill:#2e1065,stroke:#c084fc,stroke-width:2px,color:#f3e8ff;
    classDef distNode fill:#0c4a6e,stroke:#38bdf8,stroke-width:2px,color:#f0f9ff;
    classDef accessNode fill:#064e3b,stroke:#34d399,stroke-width:1.5px,color:#ecfdf5;
    classDef devNode fill:#1e293b,stroke:#94a3b8,stroke-width:1px,stroke-dasharray: 2 2,color:#e2e8f0;

    classDef coreGroup fill:#1e152f,stroke:#6b21a8,stroke-width:1px,color:#c084fc;
    classDef distGroup fill:#0b233a,stroke:#0369a1,stroke-width:1px,color:#38bdf8;
    classDef accessGroup fill:#082d24,stroke:#047857,stroke-width:1px,color:#34d399;
    classDef devGroup fill:#111827,stroke:#374151,stroke-width:1px,color:#94a3b8;

    %% 应用节点样式
    class A coreNode;
    class B1,B2 distNode;
    class C1,C2,C3,C4 accessNode;
    class D1,D2,D3,D4,D5,D6 devNode;

    %% 应用分组样式
    class Core coreGroup;
    class Dist distGroup;
    class Access accessGroup;
    class EndDev devGroup;
{% endmermaid %}
