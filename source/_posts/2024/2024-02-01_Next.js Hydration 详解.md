...
sequenceDiagram
    ...
    User->>Browser: 请求页面
    Browser->>NextServer: 发送 HTTP 请求
    
    activate NextServer
    Note over NextServer: 渲染 React 组件为静态 HTML
    NextServer->>Browser: 返回 HTML (骨架) + JS (灵魂)
    deactivate NextServer

    Note right of Browser: 关键路径：FCP (首次内容绘制)
    Browser-->>Browser: 5. 渲染静态 HTML (不可交互)
    
    rect rgb(33, 38, 45)
        Note right of Browser: 进入 Hydration 阶段
        Browser->>Browser: 6. 加载并执行 JS Bundle
        Browser->>Browser: 7. 执行水合 (Hydration)
        
        alt Mismatch (不匹配)
            Browser-->>Browser: 7a. 发现服务端与客户端 DOM 不一致
            Note over Browser: ⚠️ 触发控制台警告 & 强制重绘
        else Match (匹配)
            Browser-->>Browser: 7b. 附加事件监听器 & 初始化 State
        end
    end
