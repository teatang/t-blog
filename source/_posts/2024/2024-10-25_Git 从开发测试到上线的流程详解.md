...
*   **`main` (或 `master`) 分支**：用于存放**生产环境**稳定代码。只有当代码准备好发布时，才合并到此分支。每次合并到 `main` 都应该打上版本标签。
...
`GitHub Flow`（或其演进的 `GitLab Flow`）是一种更简洁、更轻量级的模型，适用于持续集成/持续部署 (CI/CD) 环境，通常只有一个长期分支 (`main` 或 `master`)。
