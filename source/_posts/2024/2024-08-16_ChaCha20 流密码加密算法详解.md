...
flowchart TD
    %% 输入参数构建
    subgraph Inputs [" 📥 初始输入参数 (合计 512 位) "]
        direction LR
        Const["🏷️ 常量 (128-bit)<br/><code>'expand 32-byte k'</code>"]
        Key["🔑 密钥 (256-bit)<br/><code>8 个 32位字</code>"]
        Counter["🔢 块计数器 (32-bit)<br/><code>Block Counter</code>"]
        Nonce["🎲 随机数 (96-bit)<br/><code>Nonce (3 个字)</code>"]
    end

    %% 状态矩阵初始化
    InitState["🔲 <b>初始状态矩阵 (Initial State)</b><br/>4×4 矩阵 (16 个 32位字，共 64 字节)"]
    Inputs --> InitState

    %% 核心 20 轮运算
    subgraph CoreLoop [" 🔁 20 轮四分之一轮变换 (10 次 Double-Rounds) "]
        direction TB
        ColRound["⬇️ <b>奇数轮: 列轮 (Column Round)</b><br/>对 4 列并行执行 4 次 QuarterRound (QR)"]
        DiagRound["↘️ <b>偶数轮: 对角线轮 (Diagonal Round)</b><br/>对 4 条对角线并行执行 4 次 QR"]
        
        ColRound --> DiagRound
        DiagRound -->|"迭代执行 10 次双轮"| ColRound
    end

    InitState -->|"复制为工作状态"| ColRound

    %% 馈送相加（Feedforward）
    WorkState["📋 变换后的工作状态矩阵<br/>(Working State)"]
    DiagRound -->|"完成 20 轮"| WorkState

    AddState["➕ <b>字级模加累加 (Feedforward Addition)</b><br/><code>State = Working State + Initial State (mod 2³²)</code>"]
    WorkState --> AddState
    InitState -.->|"直连馈送原始状态"| AddState

    %% 序列化为密钥流
    Serialize["🔄 序列化展平 (小端序 Little-Endian)"]
    KeyStream["✨ <b>密钥流块 (Keystream Block)</b><br/>64 字节 (512 bits)"]

    AddState --> Serialize --> KeyStream

    %% 加密运算
    Plain["📄 <b>明文块 (Plaintext)</b><br/>最多 64 字节"]
    XOR["⊕ 按位异或 (XOR)"]
    Cipher(["📦 <b>密文块 (Ciphertext)</b><br/>与明文等长"])
    ...
    style Const fill:#1e293b,stroke:#64748b,stroke-width:1.2px,color:#cbd5e1
    style Key fill:#3b1d54,stroke:#c084fc,stroke-width:1.5px,color:#f8fafc
    style Counter fill:#1e293b,stroke:#0ea5e9,stroke-width:1.2px,color:#cbd5e1
    style Nonce fill:#1e293b,stroke:#0ea5e9,stroke-width:1.2px,color:#cbd5e1

    style InitState fill:#1e3a5f,stroke:#38bdf8,stroke-width:2px,color:#ffffff
    style WorkState fill:#1e293b,stroke:#38bdf8,stroke-width:1.2px,color:#f8fafc
    style ColRound fill:#1e293b,stroke:#f59e0b,stroke-width:1.2px,color:#f8fafc
    style DiagRound fill:#1e293b,stroke:#f59e0b,stroke-width:1.2px,color:#f8fafc
    style AddState fill:#78350f,stroke:#f59e0b,stroke-width:1.5px,color:#ffffff
    style Serialize fill:#1e293b,stroke:#64748b,stroke-width:1.2px,color:#cbd5e1

    style KeyStream fill:#064e3b,stroke:#10b981,stroke-width:1.5px,color:#a7f3d0
    style Plain fill:#1e293b,stroke:#64748b,stroke-width:1.5px,color:#f8fafc
    style XOR fill:#0284c7,stroke:#38bdf8,stroke-width:2px,color:#ffffff
