...
1.  **点加法 ($P + Q = R$)**：
    ...
    **几何加法 (P + Q)**：
    ...
    flowchart LR
        ...
        %% 几何构造流水线
        Line["📏 <b>作割线 L</b><br/>穿过点 P 与 Q 的直线"]
        Intersect["✖️ <b>相交于第三点 R'</b><br/>连线交曲线于 <code>R'(x₃, y₃)</code><br/><i>(满足 P + Q + R' = 0)</i>"]
        Reflect["🪞 <b>沿 X 轴对称反射</b><br/><code>R = -R' = (x₃, -y₃)</code>"]
        Result(["✨ <b>加法结果点 R</b><br/><b>P + Q = R</b>"])
        ...
        %% 深色主题样式定制
        style Inputs fill:#0f172a,stroke:#475569,stroke-dasharray: 4 4,color:#94a3b8

        style P fill:#1e293b,stroke:#38bdf8,stroke-width:1.5px,color:#f8fafc
        style Q fill:#1e293b,stroke:#38bdf8,stroke-width:1.5px,color:#f8fafc

        style Line fill:#1e293b,stroke:#64748b,stroke-width:1.2px,color:#f8fafc
        style Intersect fill:#451a03,stroke:#f59e0b,stroke-width:1.5px,color:#fde68a
        style Reflect fill:#3b1d54,stroke:#c084fc,stroke-width:1.5px,color:#ffffff
        style Result fill:#14532d,stroke:#22c55e,stroke-width:2px,color:#f0fdf4
    {% endmermaid %}
...
2.  **点倍乘 ($P + P = 2P$)**：
    ...
    **几何倍乘 (2P)**：
    ...
    flowchart LR
        %% 输入点
        Start(["🔵 <b>输入点 P</b><br/><code>(x₁, y₁) [y₁ ≠ 0]</code>"])

        %% 几何构造流水线
        Tangent["📐 <b>作曲线切线 L</b><br/>计算斜率 <code>λ = (3x₁² + a) / (2y₁)</code>"]
        Intersect["✖️ <b>相交于另一点 R'</b><br/>切线与曲线交于 <code>R'(x₃, y₃)</code><br/><i>(满足 2P + R' = 0)</i>"]
        Reflect["🪞 <b>沿 X 轴对称反射</b><br/><code>R = -R' = (x₃, -y₃)</code>"]
        Result(["✨ <b>点倍加结果点 R</b><br/><b>2P = R</b>"])
        ...
        %% 特殊边界分支说明
        Inf(["♾️ <b>无穷远点 𝒪</b>"])
        Start -.->|"若 y₁ = 0 (垂直切线)"| Inf

        %% 深色主题样式定制
        style Start fill:#1e3a5f,stroke:#38bdf8,stroke-width:2px,color:#ffffff
        style Tangent fill:#1e293b,stroke:#0ea5e9,stroke-width:1.2px,color:#f8fafc
        style Intersect fill:#451a03,stroke:#f59e0b,stroke-width:1.5px,color:#fde68a
        style Reflect fill:#3b1d54,stroke:#c084fc,stroke-width:1.5px,color:#ffffff
        style Result fill:#14532d,stroke:#22c55e,stroke-width:2px,color:#f0fdf4
        style Inf fill:#1e293b,stroke:#64748b,stroke-dasharray: 3 3,color:#94a3b8
    {% endmermaid %}
...
flowchart TD
    %% 阶段 1：域参数
    subgraph DomainParams [" 🌐 椭圆曲线域参数 (Domain Parameters) "]
        direction TB
        Params["<b>标准曲线六元组 (p, a, b, G, n, h)</b><br/><code>y² ≡ x³ + ax + b (mod p)</code><br/>基点: <code>G</code> · 基点阶: <code>n</code>"]
    end

    %% 阶段 2：私钥抽取
    subgraph PrivateGen [" 🔐 私钥生成 (安全随机源 CSPRNG) "]
        direction TB
        PrivKey["<b>生成随机私钥 d</b><br/>标量整数: <code>1 ≤ d &lt; n</code>"]
    end

    Params --> PrivKey

    %% 阶段 3：标量乘法
    CalcPub["⚡ <b>椭圆曲线标量乘法 (点乘)</b><br/><code>Q = [d]G = G + G + ... + G (d 次)</code><br/><i>(基于 Double-and-Add 等快速算法)</i>"]
    PrivKey --> CalcPub
    Params -.->|"提供基点 G"| CalcPub

    %% 阶段 4：密钥对输出
    subgraph KeyPair [" 📦 生成结果：非对称密钥对 "]
        direction LR
        PubOut["🌐 <b>公钥 Q (Public Key)</b><br/>曲线上的坐标点 <code>(x_Q, y_Q)</code><br/>🟢 <b>完全公开共享</b>"]
        PrivOut["🔒 <b>私钥 d (Private Key)</b><br/>大整数标量<br/>🔴 <b>硬件隔离/绝密保存</b>"]
    end
    ...
    %% 深色主题样式定制
    style DomainParams fill:#0f172a,stroke:#64748b,stroke-dasharray: 4 4,color:#cbd5e1
    style PrivateGen fill:#1e1035,stroke:#a855f7,stroke-dasharray: 4 4,color:#d8b4fe
    style KeyPair fill:#090d16,stroke:#334155,stroke-dasharray: 3 3,color:#94a3b8

    style Params fill:#1e293b,stroke:#64748b,stroke-width:1.2px,color:#f8fafc
    style PrivKey fill:#3b1d54,stroke:#c084fc,stroke-width:1.5px,color:#ffffff
    style CalcPub fill:#451a03,stroke:#f59e0b,stroke-width:1.5px,color:#fde68a

    style PubOut fill:#0369a1,stroke:#38bdf8,stroke-width:2px,color:#ffffff
    style PrivOut fill:#581c87,stroke:#c084fc,stroke-width:2px,color:#ffffff
{% endmermaid %}
...
flowchart TD
    %% 阶段 1：公共域参数
    subgraph Params [" 🌐 椭圆曲线域参数 (公开规范) "]
        P["曲线六元组: <b>(p, a, b, G, n, h)</b><br/>基点: <code>G</code> · 基点阶: <code>n</code>"]
    end

    %% 阶段 2：双方私钥与公钥生成
    subgraph Alice [" 👩‍💻 Alice (本地安全域) "]
        direction TB
        PrivA["🔐 随机私钥 <b>d_A</b><br/><code>1 ≤ d_A &lt; n</code>"]
        PubA["📤 点乘生成公钥<br/><code>Q_A = [d_A]G</code>"]
        PrivA --> PubA
    end

    subgraph Bob [" 👨‍💻 Bob (本地安全域) "]
        direction TB
        PrivB["🔐 随机私钥 <b>d_B</b><br/><code>1 ≤ d_B &lt; n</code>"]
        PubB["📤 点乘生成公钥<br/><code>Q_B = [d_B]G</code>"]
        PrivB --> PubB
    end
    ...
    %% 阶段 3：公开信道与窃听者
    subgraph Channel [" 📡 公开传输信道 (Insecure Network) "]
        direction LR
        ChanA["公钥 <b>Q_A</b> 传输"]
        ChanB["公钥 <b>Q_B</b> 传输"]
    end

    subgraph Eve [" 🕵️‍♀️ 窃听者 Eve (旁路嗅探) "]
        direction TB
        Eve_Obs["👀 截获公开信息:<br/><code>Params, Q_A, Q_B</code>"]
        Eve_ECDLP["🛑 <b>ECDLP 难题困境</b><br/>无法在多项式时间内从<br/>Q_A 或 Q_B 反解出 d_A 或 d_B"]
        Eve_Obs --> Eve_ECDLP
    end

    PubA -->|"发送 Q_A"| ChanA -->|"接收 Q_A"| BobCalc
    PubB -->|"发送 Q_B"| ChanB -->|"接收 Q_B"| AliceCalc
    ...
    %% 阶段 4：计算共享秘密
    AliceCalc["🧮 计算共享点<br/><code>S = [d_A]Q_B</code>"]
    BobCalc["🧮 计算共享点<br/><code>S = [d_B]Q_A</code>"]

    PrivA --> AliceCalc
    PrivB --> BobCalc

    %% 最终达成
    SharedPoint(["✨ <b>一致的共享秘密点 S</b><br/><code>S = [d_A · d_B]G = (x_S, y_S)</code>"])
    AliceCalc --> SharedPoint
    BobCalc --> SharedPoint

    FinalKDF["🔑 <b>衍生会话密钥 K</b><br/><code>K = KDF(x_S)</code> 用于后续对称加密"]
    SharedPoint --> FinalKDF
    ...
    style Channel fill:#0f172a,stroke:#334155,stroke-width:1.5px,color:#94a3b8
    ...
    style Eve_ECDLP fill:#1f1315,stroke:#b91c1c,color:#fca5a5
    ...
    style SharedPoint fill:#14532d,stroke:#22c55e,stroke-width:2px,color:#f0fdf4
    style FinalKDF fill:#134e4a,stroke:#14b8a6,stroke-width:1.5px,color:#ccfbf1
{% endmermaid %}
