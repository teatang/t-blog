...
{% mermaid %}
flowchart TD
    subgraph StateIn [" 📥 当前轮工作变量 (Round t Input) "]
        direction LR
        In_A["a"] ~~~ In_B["b"] ~~~ In_C["c"] ~~~ In_D["d"]
        In_D ~~~ In_E["e"] ~~~ In_F["f"] ~~~ In_G["g"] ~~~ In_H["h"]
        In_H ~~~ In_K["Kₜ"] ~~~ In_W["Wₜ"]
    end

    %% T2 计算流
    subgraph CalcT2 [" 🔷 计算 T₂ "]
        direction TB
        Sig0["🔀 Σ₀(a)<br/><code>ROTR² ⊕ ROTR¹³ ⊕ ROTR²²</code>"]
        Maj["⚖️ Maj(a, b, c)<br/><code>(a∧b) ⊕ (a∧c) ⊕ (b∧c)</code>"]
        SumT2["➕ 模加: <b>T₂ = Σ₀ + Maj</b>"]
    ...
    %% T1 计算流
    subgraph CalcT1 [" 🔶 计算 T₁ "]
        direction TB
        Sig1["🔀 Σ₁(e)<br/><code>ROTR⁶ ⊕ ROTR¹¹ ⊕ ROTR²⁵</code>"]
        Ch["🔀 Ch(e, f, g)<br/><code>(e∧f) ⊕ (¬e∧g)</code>"]
        SumT1["➕ 模加: <b>T₁ = h + Σ₁ + Ch + Kₜ + Wₜ</b>"]
    ...
    %% 下一轮状态集合
    subgraph StateOut [" 📤 下一轮输入状态 (Round t+1 Output) "]
        direction LR
        Out_A["a' = 新 a"]
        Out_B["b' = a"]
        Out_C["c' = b"]
    ...
    %% 深色主题样式
    style StateIn fill:#0f172a,stroke:#475569,stroke-dasharray: 4 4,color:#94a3b8
    style StateOut fill:#0f172a,stroke:#22c55e,stroke-dasharray: 4 4,color:#86efac
    style CalcT1 fill:#171625,stroke:#f59e0b,stroke-width:1.5px,color:#fde68a
    style CalcT2 fill:#0c1a2e,stroke:#38bdf8,stroke-width:1.5px,color:#bae6fd

    style NewA fill:#1e3a5f,stroke:#38bdf8,stroke-width:2px,color:#ffffff
    style NewE fill:#451a03,stroke:#f59e0b,stroke-width:2px,color:#ffffff

    style Sig0 fill:#1e293b,stroke:#0ea5e9,color:#f8fafc
    style Maj fill:#1e293b,stroke:#0ea5e9,color:#f8fafc
    style SumT2 fill:#075985,stroke:#38bdf8,color:#ffffff

    style Sig1 fill:#1e293b,stroke:#f59e0b,color:#f8fafc
    style Ch fill:#1e293b,stroke:#f59e0b,color:#f8fafc
    style SumT1 fill:#92400e,stroke:#fbbf24,color:#ffffff

    style Out_A fill:#14532d,stroke:#22c55e,color:#ffffff
    style Out_E fill:#14532d,stroke:#22c55e,color:#ffffff
{% endmermaid %}
