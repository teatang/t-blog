...
flowchart LR
    %% 主流程：数据平面转发流程
    subgraph ForwardingPath["数据平面：三层转发处理流程 (Forwarding Engine)"]
        A(["数据包进入路由器接口"]) --> B["解析报头：提取目的 IP 地址"]
        B --> C{"查找路由表<br/>(最长前缀匹配 LPM)"}
        
        C -- "匹配失败 (无默认路由)" --> H["丢弃报文<br/>发送 ICMP 目标不可达"]
        C -- "匹配成功" --> D["确定出接口与下一跳 IP"]
        
        D --> E{"校验并递减 TTL<br/>(TTL - 1)"}
        E -- "TTL <= 0" --> E_Drop["丢弃报文<br/>发送 ICMP 超时 (TTL Expired)"]
        E -- "TTL > 0" --> F["重新封装数据链路层<br/>(ARP 解析下一跳 MAC)"]
        
        F --> G(["从目标出接口发送报文"])
    end

    %% 辅助流程：控制平面路由来源
    subgraph ControlPlane["控制平面：路由表来源 (Routing Table Sources)"]
        RT_Head["路由管理进程 (RIB)"]
        ...
        RT3_1["IGP (内部网关协议)<br/>OSPF / IS-IS / EIGRP"]
        RT3_2["EGP (外部网关协议)<br/>BGP"]
    ...
    %% 控制平面向数据平面注入 FIB
    RT_Head -. "下发转发表项 (FIB)" .-> C

    %% 节点样式分类 (Fill, Stroke, Color)
    classDef normalNode fill:#0c4a6e,stroke:#38bdf8,stroke-width:1.5px,color:#f0f9ff;
    classDef decisionNode fill:#3b0764,stroke:#c084fc,stroke-width:2px,color:#f3e8ff;
    classDef successNode fill:#064e3b,stroke:#34d399,stroke-width:2px,color:#ecfdf5;
    classDef failNode fill:#450a0a,stroke:#f87171,stroke-width:2px,color:#fef2f2;
    classDef ribNode fill:#1e293b,stroke:#f59e0b,stroke-width:1.5px,color:#fef3c7;
    classDef protoNode fill:#182234,stroke:#94a3b8,stroke-width:1px,stroke-dasharray: 2 2,color:#cbd5e1;

    %% 容器样式
    classDef groupFwd fill:#0b1d33,stroke:#0284c7,stroke-width:1.5px,color:#38bdf8;
    classDef groupCtrl fill:#171d2c,stroke:#d97706,stroke-width:1.5px,color:#fbbf24;

    %% 应用节点样式
    class A,B,D,F normalNode;
    class C,E decisionNode;
    class G successNode;
    class H,E_Drop failNode;
    class RT_Head,RT1,RT2,RT3 ribNode;
    class RT3_1,RT3_2 protoNode;

    %% 应用容器样式
    class ForwardingPath groupFwd;
    class ControlPlane groupCtrl;
{% endmermaid %}
