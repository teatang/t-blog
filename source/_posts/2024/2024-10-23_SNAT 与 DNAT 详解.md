...
graph TD
    %% 样式定义
    classDef internal fill:#1e272e,stroke:#05c46b,stroke-width:2px,color:#d2dae2;
    classDef router fill:#1e272e,stroke:#f85149,stroke-width:2px,color:#f85149;
    classDef external fill:#1e272e,stroke:#58a6ff,stroke-width:2px,color:#d2dae2;
    classDef table fill:#21262d,stroke:#d29922,stroke-width:1px,color:#f8e3a1,font-size:12px;

    subgraph LAN [私网区域 - Internal]
        Host["💻 内部主机<br/>192.168.1.10:12345"]
    end

    subgraph GW [边界网关 - NAT Device]
        direction TB
        PublicIP["🌐 公网出口<br/>203.0.113.1"]
        Table["📝 SNAT/PAT 映射表<br/>[192.168.1.10:12345] ↔ [Port: 20001]"]
    end

    subgraph WAN [公网区域 - Internet]
        Target["☁️ 外部服务器<br/>8.8.8.8:80"]
    end
...
graph TD
    %% 样式定义：适配深色模式
    classDef client fill:#1e272e,stroke:#58a6ff,stroke-width:2px,color:#d2dae2;
    classDef firewall fill:#1e272e,stroke:#f85149,stroke-width:2px,color:#f85149;
    classDef server fill:#1e272e,stroke:#05c46b,stroke-width:2px,color:#d2dae2;
    classDef table fill:#21262d,stroke:#d29922,stroke-width:1px,color:#f8e3a1,font-size:12px;

    subgraph WAN [公网区域 - Internet]
        Client["💻 外部客户端<br/>1.1.1.1:50000"]
    end

    subgraph GW [边界网关 - NAT Device]
        direction TB
        V-IP["🌐 公网服务入口<br/>203.0.113.1:80"]
        Table["📝 DNAT 映射表<br/>[Public:80] ↔ [Internal:80]"]
    end

    subgraph LAN [私网区域 - Internal]
        WebServer["🚀 Web 服务器<br/>192.168.1.100:80"]
    end

    %% 流程线：使用不同线形增强辨识度
    Client ==>|1. 访问公网 IP| V-IP
    V-IP ==>|2. 目的地址转换| WebServer
    
    WebServer -.->|3. 发回响应数据| V-IP
    V-IP -.->|4. 源地址还原| Client
...
{% mermaid %}
%%{init: {'theme': 'dark', 'themeVariables': { 'sequenceNumberColor': '#fff', 'actorLineColor': '#58a6ff', 'actorBorder': '#30363d', 'actorBkg': '#161b22', 'labelBoxBorderColor': '#30363d', 'labelBoxBkgColor': '#0d1117', 'activationBorderColor': '#1f6feb', 'activationBkgColor': '#1f6feb', 'noteBkgColor': '#21262d', 'noteBorderColor': '#d29922' }}}%%
sequenceDiagram
    autonumber
    
    participant Ext as 外部客户端<br/>1.1.1.1
    participant GW as 边界防火墙 (NAT)<br/>203.0.113.1
    participant Int as 内部服务器<br/>192.168.1.100

    Note over Ext, GW: 【公网区域】
    Note over Int: 【私网区域】

    Ext->>GW: 1. 请求访问 (Dest: 203.0.113.1:80)
    
    rect rgb(33, 38, 45)
        activate GW
        Note over GW: 触发 DNAT 规则
        GW-->>GW: 修改目的 IP -> 192.168.1.100<br/>记录 Session 会话表
        GW->>Int: 2. 转发请求 (Dest: 192.168.1.100:80)
        deactivate GW
    end

    Int->>Int: 处理业务逻辑

    Int->>GW: 3. 响应回包 (Source: 192.168.1.100:80)

    rect rgb(33, 38, 45)
        activate GW
        Note over GW: 查找会话表 (逆向转换)
        GW-->>GW: 修改源 IP -> 203.0.113.1<br/>(SNAT 还原)
        GW->>Ext: 4. 返回响应 (Source: 203.0.113.1:80)
        deactivate GW
    end
{% endmermaid %}
